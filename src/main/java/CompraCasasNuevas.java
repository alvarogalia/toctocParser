...
public class CompraCasasNuevas extends Thread{
    public void run() {
        try (BufferedReader br = new BufferedReader(new FileReader("sh\\salida_compra_casas_nuevas.json"))) {
            File fout = new File("salida_compra_casas_nuevas.json");
            File foutcsv = new File("salida_compra_casas_nuevas.csv");
            File foutError = new File("salida_compra_casas_nuevas_error.json");
