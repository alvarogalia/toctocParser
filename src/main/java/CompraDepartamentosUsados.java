...
public class CompraDepartamentosUsados extends Thread {
    public void run() {
        try (BufferedReader br = new BufferedReader(new FileReader("sh\\salida_compra_departamentos_usados.json"))) {
            File fout = new File("out/salida_compra_departamentos_usados.json");
            File foutcsv = new File("out/salida_compra_departamentos_usados.csv");
            File foutError = new File("out/salida_compra_departamentos_usados_error.json");
            ...
            for (int i = 0; i < arr.length(); i++) {
                ...
                try {
                    ...
                    if (status == 200 || status == 201) {
                        ...
                        if (!JSONObject.NULL.equals(jsonProperties.get("UriFicha"))) {
                            UriFicha = jsonProperties.getString("UriFicha");
                        }
                        if (!JSONObject.NULL.equals(jsonProperties.get("Data"))) {
                            Data = jsonProperties.getJSONObject("Data");
                            if (!JSONObject.NULL.equals(Data.get("Propiedad"))) {
                                Propiedad = Data.getJSONObject("Propiedad");
                                if (!JSONObject.NULL.equals(Propiedad.get("MetrosUtiles"))) {
                                    MetrosUtiles = Propiedad.getDouble("MetrosUtiles");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("MetrosTerraza"))) {
                                    MetrosTerraza = Propiedad.getDouble("MetrosTerraza");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("MetrosConstruidos"))) {
                                    MetrosConstruidos = Propiedad.getDouble("MetrosConstruidos");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("MetrosTerreno"))) {
                                    MetrosTerreno = Propiedad.getDouble("MetrosTerreno");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Dormitorios"))) {
                                    Dormitorios = Propiedad.getInt("Dormitorios");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Banos"))) {
                                    Banos = Propiedad.getInt("Banos");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("AñoConstruccion"))) {
                                    AnoConstruccion = Propiedad.getInt("AñoConstruccion");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("NombreCorredora"))) {
                                    NombreCorredora = Propiedad.getString("NombreCorredora");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Id"))) {
                                    Id = Propiedad.getInt("Id");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("EstaPublicada"))) {
                                    EstaPublicada = Propiedad.getBoolean("EstaPublicada");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("EstaEliminada"))) {
                                    EstaEliminada = Propiedad.getBoolean("EstaEliminada");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("PrecioDesde"))) {
                                    PrecioDesde = Propiedad.getDouble("PrecioDesde");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("PrecioHasta"))) {
                                    PrecioHasta = Propiedad.getDouble("PrecioHasta");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("PrecioUF"))) {
                                    PrecioUF = Propiedad.getDouble("PrecioUF");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("PrecioPesos"))) {
                                    PrecioPesos = Propiedad.getDouble("PrecioPesos");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Comuna"))) {
                                    Comuna = Propiedad.getString("Comuna");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Region"))) {
                                    Region = Propiedad.getString("Region");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("TipoOperacion"))) {
                                    TipoOperacion = Propiedad.getString("TipoOperacion");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Latitud"))) {
                                    Latitud = Propiedad.getDouble("Latitud");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("Longitud"))) {
                                    Longitud = Propiedad.getDouble("Longitud");
                                }
                                if (!JSONObject.NULL.equals(Propiedad.get("FechaPublicacionDespliegue"))) {
                                    FechaPublicacionDespliegue = Propiedad.getString("FechaPublicacionDespliegue");
                                }
                            }
                            if (!JSONObject.NULL.equals(Data.get("Estadistica"))) {
                                Estadistica = Data.getJSONObject("Estadistica");
                                if (!JSONObject.NULL.equals(Estadistica.get("CantidadVisitas"))) {
                                    CantidadVisitas = Estadistica.getInt("CantidadVisitas");
                                }
                                if (!JSONObject.NULL.equals(Estadistica.get("CantidadFavoritos"))) {
                                    CantidadFavoritos = Estadistica.getInt("CantidadFavoritos");
                                }
                                if (!JSONObject.NULL.equals(Estadistica.get("CantidadInteresados"))) {
                                    CantidadInteresados = Estadistica.getInt("CantidadInteresados");
                                }
                            }
                            if (!JSONObject.NULL.equals(Data.get("Exitosa"))) {
                                Exitosa = Data.getBoolean("Exitosa");
                            }
                        }
                    ...
                    bwError.write(id + "->" + e.getMessage());
                    System.out.println(this.getClass().getName() + " Error " + i + ":" + e.getMessage());
                }
